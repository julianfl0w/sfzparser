...
class SFZInstrument:
    def __init__(
        ...
        self.sfzFilename = sfzFilename
        self.sfzFilenameBasedir = os.path.dirname(sfzFilename)
        self.samplesBasedir = Path(samplesBasedir).resolve()
        # see if we have already converted the wavs to necessary binary format

        # check if binary file has already been exported
        
        binFilename = os.path.join(self.sfzFilenameBasedir, "_" + platform_simple + ".bin")
        if os.path.exists(binFilename):
            with open(binFilename, "rb") as f:
                self.sampleFilename2address, self.sampleFilename2lengthSamples, self.binaryBlob = pickle.load(
                    f
                )
        # otherwise create it!
        else:
            ...
            self.allSampleFilenames = [
                str(s.resolve()) for s in list(Path(samplesBasedir).rglob("*.wav"))
            ]
            self.allSampleFilenames += [
                str(s.resolve()) for s in list(Path(samplesBasedir).rglob("*.flac"))
            ]
        ...
        self.loadSFZ(sfzFilename=sfzFilename)
    ...
    def preprocess(self, sfzFilename, replaceDict = {}):
        ...
        for ogline in preprocFile.split("\n"):
            ...
            if ogline.startswith("//"):
                ...
                for includeFile in line.split("#include")[1:][::2]:
                    includeFilename = eval(includeFile.strip())
                    includeFilename = os.path.join(self.sfzFilenameBasedir, includeFilename)
                    includeText = self.preprocess(includeFilename, replaceDict=replaceDict.copy())
                    preProcessText += "\n" + includeText + "\n"
    ...
    def loadSFZ(self, sfzFilename, depth=0):
        ...
        def noteInSfzRegion(noteNo, region):
            inRegion = True
            if "lokey" in region.keys() and noteNo < eval(region["lokey"]):
                inRegion = False
            if "hikey" in region.keys() and noteNo > eval(region["hikey"]):
                inRegion = False
            return inRegion
